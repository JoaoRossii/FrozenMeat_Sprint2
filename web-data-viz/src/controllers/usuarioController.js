...
function cadastrar(req, res) {
  ...
  if (nome == undefined) {
    ...
    res.status(400).send("Seu CPF/CNPJ está undefined!");
    ...
    usuarioModel
      .cadastrar(nome, cpfCnpj, cargo, email, telefone, senha)
...
function autenticar(req, res) {
  ...
  if (cpfCnpj == undefined) {
    res.status(400).send("Seu CPF ou CNPJ está indefinido!");
  } else if (senha == undefined) {
    res.status(400).send("Sua senha está indefinida!");
  } else {
    usuarioModel
      .autenticar(cpfCnpj, senha)
      .then(function (resultadoAutenticar) {
        ...
        if (resultadoAutenticar.length == 1) {
          ...
          res.status(403).send("CPF/CNPJ ou senha inválido(s)");
        } else {
...
function foraFaixa(req, res) {
  ...
  if (idUsuario == undefined) {
    ...
    usuarioModel
      ...
      .catch(function (erro) {
        console.log("\nHouve um erro ao puxar os dados! Erro: ", erro);
        res.status(500).json(erro.sqlMessage);
      });
  }
}
